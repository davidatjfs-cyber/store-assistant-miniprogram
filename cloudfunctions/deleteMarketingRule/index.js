...
async function getActiveStaffByOpenid(openid) {
  ...
  const r = await db.collection('staff').where({ openid, active: true }).limit(1).get();
...
function normalizeRole(row) {
  ...
  if (!['staff', 'manager', 'admin'].includes(role)) role = 'staff';
  return role;
}
...
exports.main = async (event, context) => {
  ...
  if (!ruleId) return { success: false, message: '缺少 rule_id' };

  try {
    const staff = await getActiveStaffByOpenid(OPENID);
    if (normalizeRole(staff) !== 'admin') return { success: false, message: '无权限' };
    ...
    if (msg.indexOf('cannot find document') !== -1 || msg.indexOf('不存在') !== -1) {
      return { success: false, message: '规则不存在或已被删除' };
    }
